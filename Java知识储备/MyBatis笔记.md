...
## MyBatis特性
1. MyBatis 是支持**定制化 SQL**、存储过程以及**高级映射**的优秀的持久层框架
...
工程名→打包方式→添加依赖(定位)→添加jdbc.properties与log4j.xml文件→创建Mybatis核心配置文件（mybatis-config.xml）→设置类型别名包与映射文件包
...
- ![image-20221015190436015](D:\hqq\Java\尚硅谷\MyBatis\MyBatis笔记.assets\image-20221015190436015.png)
## 默认的类型别名
...
- 注意：
	...
	1. 查询的标签select必须设置属性resultType或resultMap，用于设置实体类和数据库表的映射关系  
		...
		- resultMap：自定义映射，用于一对多或多对一或字段名和属性名不一致的情况  （设置自定义的映射关系）
...
- 实体类对象的属性和属性值，表示实体类中get,set方法中的名字
```xml
...
```
## 5、使用@Param标识参数（使用@Param注解 命名参数）
 ...
 * 查询用户的总记录数  
 * @return  
 * 在MyBatis中，对于Java中常用的类型都设置了默认类型别名  
...
**搭建MyBatis框架：工程名→打包方式→添加依赖(定位)→添加jdbc.properties与log4j.xml文件→创建Mybatis核心配置文件（mybatis-config.xml）→设置类型别名包与映射文件包（映射文件包中的xml文件的命名空间要和映射接口包中的文件位置对应）**

![image-20221022092921283](MyBatis笔记.assets/image-20221022092921283.png)
...
- 设置全局配置，将 _ 自动映射为驼峰
  - `<setting name="mapUnderscoreToCamelCase" value="true"/>`
- **通过resultMap处理字段和属性的自定义映射关系**

## 1、resultMap处理字段和属性的映射关系
- resultMap：设置**自定义映射**  （属性驼峰命名、字段加下划线命名）
	- 属性：  
		- resultMap id：表示**自定义映射的唯一标识**，不能重复
	- type：查询的数据要映射的**实体类的类型**  
	- 子标签：  
	  - id：设置**主键的映射**关系  
	  - result：设置**普通字段的映射**关系  
	  - 子标签属性：  
	    - property：设置映射关系中实体类中的属性名，必须是type属性所设置的实体类类型中的属性名
	    - column：设置映射关系中表中的字段名，必须是sql语句查询出的字段名
- 若字段名和实体类中的属性名不一致，则可以通过resultMap设置自定义映射，**即使字段名和属性名一致的属性也要映射**，也就是**全部属性都要列出来**
```xml
...
## 1、if
- if标签可通过test属性（即传递过来的数据）的表达式进行判断，若表达式的结果为true，则标签中的内容会执行；反之标签中的内容不会执行（**根据标签中的test属性所对应的表达式决定标签中的内容是否需要拼接到SQL中**）
...
**当where标签中有内容时，会自动生成关键字where，并且将内容前多余的and或or去掉；**
...
```
![image-20221022191812747](MyBatis笔记.assets/image-20221022191812747.png)
...
![image-20221022192219115](MyBatis笔记.assets/image-20221022192219115.png)
...
```
![image-20221022192612915](MyBatis笔记.assets/image-20221022192612915.png)
...
sqlSessoin.clearCache();//清空缓存
```
## 2、MyBatis的二级缓存
- **二级缓存是SqlSessionFactory级别**，通过同一个SqlSessionFactory创建的SqlSession查询的结果会被缓存；此后若再次执行相同的查询语句，结果就会从缓存中获取  
- 二级缓存开启的条件：(四个条件缺一不可)

	1. 在核心配置文件中，设置全局配置属性（settings标签）cacheEnabled="true"，默认为true，不需要设置
	2. 在映射文件中设置标签<cache />
	3. 二级缓存必须在SqlSession关闭或提交之后有效 sqlSession.close()或sqlSession.commit();
...
### 创建EHCache的配置文件ehcache.xml(resources下)
- 名字必须叫`ehcache.xml`
```xml
<?xml version="1.0" encoding="utf-8" ?>
...
#### 方法二使用PageInfo
- 在**查询获取list集合之后**，使用`PageInfo<T> pageInfo = new PageInfo<>(List<T> list, intnavigatePages)`获取分页相关数据
  - list：分页之后的数据  
...
>  资料来源：[01_MyBatis课程介绍_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1VP4y1c7j7?p=1&vd_source=817119647dd4820bfbb8128007f410e6)